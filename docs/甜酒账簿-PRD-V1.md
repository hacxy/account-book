...
| 模块 | 状态 | 说明 |
|------|------|------|
| 基础框架 & 导航容器 | ✅ 已完成 | `PageContainer` 组件，自定义导航栏，状态栏高度适配 |
| 首页框架 | ✅ 已完成 | 顶部导航（头像 + 标题）、FAB 悬浮按钮 |
| 我的页 | ✅ 已完成 | 用户信息展示、功能入口列表（分类管理、设置） |
| 快速记账抽屉 | ✅ 已完成 | 收支切换、金额输入、自定义键盘、分类选择、日期、备注、前端校验；**后端保存未接** |
| 云函数 & 登录 | 🚧 部分完成 | `login` 云函数已接通（含完整 user 字段写入），`callFn` 封装完成；账单 CRUD 云函数待开发 |
| 首页账单列表 | ⬜ 待开发 | 当月总览卡片、按天分组账单列表、月份切换、分类筛选 |
| 账单详情与编辑 | ⬜ 待开发 | 查看、编辑、删除（含二次确认） |
| 统计页 | ⬜ 待开发 | 汇总卡片、分类占比饼图、收支趋势折线图 |
| 分类管理页 | ⬜ 待开发 | 系统分类展示、自定义分类增删停用 |
| 设置页 | ⬜ 待开发 | 昵称、货币、导出 CSV、记账提醒 |
...
> 🟢 **框架已完成**：`PageContainer` 自定义导航栏、首页头像入口、`pages/profile` 页面均已实现；统计入口待实现。
...
| 页面 | 入口 | 核心内容 | 状态 |
|------|------|----------|------|
| 首页 | 启动默认页 | 当月总览卡片、最近账单列表（按天分组）、FAB 悬浮按钮 | 🚧 框架完成，列表待开发 |
| 记一笔（抽屉） | 首页 FAB 悬浮按钮 | 金额输入、收支切换、分类、日期、备注、保存 | 🚧 前端完成，后端保存待接 |
| 账单详情页 | 首页账单列表 → 点击单条 | 查看、编辑、删除 | ⬜ 待开发 |
| 统计页 | 首页右上角"统计"入口 | 汇总卡片、支出分类占比饼图、趋势折线图 | ⬜ 待开发 |
| 分类管理页 | "我的" → 分类管理 | 分类列表、新增、编辑、停用 | ⬜ 待开发 |
| 设置页 | "我的" → 设置 | 昵称、货币、提醒设置、导出数据 | ⬜ 待开发 |
| 我的页 | 首页左上角用户头像 | 用户信息、功能入口聚合 | ✅ 已完成（基础结构） |
...
> 🚧 **前端已完成，后端保存待接入**
> - ✅ 收入/支出切换、金额输入（含自定义键盘、两位小数限制）、分类选择（系统预置 15 支出 + 6 收入）、日期选择、备注输入、前端校验
> - ✅ 以抽屉形式（`AddBillDrawer`）从首页 FAB 唤起，含进出场动画
> - ⬜ 保存至云端（账单 CRUD 云函数）待开发
...
> 🚧 **部分完成**：系统预置分类数据已在 `AddBillDrawer` 中硬编码使用（15 支出 + 6 收入）；分类管理页面待开发。
...
> 用户身份与用户设置合并为一张表，CloudBase `user` 集合。

```typescript
interface User {
  _id: string;                   // CloudBase 自动生成
  _openid: string;               // CloudBase 安全字段，自动写入
  openid: string;                // 显式冗余，供服务端云函数查询
  nickname: string;              // 昵称，默认为 openid 短码（2-6 位 base36）
  avatarUrl: string;             // 头像 URL，默认 ''，用户主动授权后更新
  reminderEnabled: boolean;      // 记账提醒开关，默认 false
  reminderTime: string;          // 提醒时间 HH:mm，默认 '21:00'
  reminderSubscribed: boolean;   // 微信订阅消息是否已授权，默认 false
  createdAt: Date;               // CloudBase 自动写入
  updatedAt: Date;               // CloudBase 自动写入
}
```

> V1 仅支持人民币，不存 `currency` 字段，展示层直接硬编码 `¥`。

> `lastCategoryId`（最近选中分类）不入库，前端用 `Taro.setStorageSync` 本地缓存。
